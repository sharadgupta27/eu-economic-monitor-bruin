"""@bruin
name: eurostat_raw.gdp_annual
type: python
connection: duckdb-local

description: |
  Ingests GDP annual data (nama_10_gdp) from Eurostat API
...
# Resolve DB path: 4 levels up from this file = project root
...
def run():
    ...
    print(f"Fetched {len(df)} rows from Eurostat GDP dataset")

    conn = connect_with_retry(DB_PATH)
    conn.execute("CREATE SCHEMA IF NOT EXISTS eurostat_raw")
    conn.execute("CREATE OR REPLACE TABLE eurostat_raw.gdp_annual AS SELECT * FROM df")
    count = conn.execute("SELECT COUNT(*) FROM eurostat_raw.gdp_annual").fetchone()[0]
    conn.close()
    print(f"Loaded {count} rows into eurostat_raw.gdp_annual")
