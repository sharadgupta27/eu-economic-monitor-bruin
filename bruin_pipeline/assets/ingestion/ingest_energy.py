"""@bruin
name: eurostat_raw.energy_intensity
type: python
connection: duckdb-local

description: |
  Ingests energy intensity of the economy (nrg_ind_ei) from Eurostat API
...
def run():
    ...
    print(f"Fetched {len(df)} rows from Eurostat energy intensity dataset")

    conn = connect_with_retry(DB_PATH)
    conn.execute("CREATE SCHEMA IF NOT EXISTS eurostat_raw")
    conn.execute("CREATE OR REPLACE TABLE eurostat_raw.energy_intensity AS SELECT * FROM df")
    count = conn.execute("SELECT COUNT(*) FROM eurostat_raw.energy_intensity").fetchone()[0]
    conn.close()
    print(f"Loaded {count} rows into eurostat_raw.energy_intensity")
...
def materialize() -> pd.DataFrame:
    """Fetch energy intensity data from Eurostat API."""
    df = fetch_to_long(
        "nrg_ind_ei",
        {"geo": COUNTRIES, "unit": ["KGOE_TEUR"]},
    )
